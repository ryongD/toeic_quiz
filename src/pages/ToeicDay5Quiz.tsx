            ...
            {submitted && (
              <div className="mt-3 border-t pt-3 text-sm space-y-2">
                {answers[q.key] === q.answer ? (
                  <div className="text-green-600 font-medium">⭕ 정답입니다!</div>
                ) : (
                  <div className="text-red-600 font-medium">
                    ❌ 오답입니다. <span className="text-green-700">정답: {q.answer}</span>
                  </div>
                )}
                {q.explanation && (
                  <div className="text-gray-700">💡 해설: {q.explanation}</div>
                )}
                {showTranslation && q.translation && (
                  <div className="text-gray-600">📝 해석: {q.translation}</div>
                )}
              </div>
            )}
          </div>
